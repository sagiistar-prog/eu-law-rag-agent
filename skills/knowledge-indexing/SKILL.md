...
8. For an explicitly requested reranking experiment, read `docs/reranker-experiment.md` and `knowledge/evaluation/reranker-protocol.md`. Run `knowledge/evaluate_review.py` against development, known probes and the checksum-frozen holdout, then `knowledge/assess_reranker.py`. Report the adoption decision separately from successful execution. Keep the default hybrid mode: the current candidate failed its holdout target. Do not retune on the observed holdout and call it independent validation.
9. For chunk/context experiments, read `knowledge/evaluation/indexing-protocol.md` and `docs/indexing-experiment.md`. Use `--chunker paragraphs --document-context source-section` only explicitly; compare identical source snapshots with `evaluate_indexing.py`, then apply `assess_indexing.py`. Both current candidates failed; retain default character chunks without metadata context. All existing case sets are now observed regression sets.
10. Check exact citation spans against the cleaned source. `content_sha256` describes emitted text, not a larger hidden passage. A truncated quote has its original passage in `excerpt_of`; never omit the partial-excerpt notice. New character indexes use v2 offsets; old v1 positions are repaired only when the original 320-character window proves the same text. Rebuild any other inconsistent index.
